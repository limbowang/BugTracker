...
@section('content')
{{ Form::model($bug, array('method'=>'PUT', 'route' => array('bug.update', $bug->id), 'files' => true, 'class' => 'form-container signin-form')) }}
...
<div class="form-group">
    ...
    <div class="form-upload">
        ...
        @else
        <input type="button" class="btn btn-info btn-upload" value="添加"/>
        @endif
        <div>
            选择的图片：<span class="filename">无</span>
        </div>
        <div class='form-error-msg'>{{ $errors->get('avatar')[0] or '' }}</div>
        {{ Form::file('img', array('id' => 'bug-img', 'class' => 'hidden input-img-upload')) }}
    </div>
